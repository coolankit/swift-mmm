    ...
    return html.Div(
        ...
        children=[
            html.Div(
                id="banner-text",
                children=[
                    html.H5("MMM Automation Dashboard"),
...
#                            selected_className="custom-tab--selected",),
                    dcc.Tab(id="Control-chart-tab",label="Post Model Dashboard",value="tab2",className="custom-tab",
                        selected_className="custom-tab--selected",),
                    dcc.Tab(id='Tab-1',label='Tab One', value='tab-1-example',className="custom-tab",selected_className="custom-tab--selected",),
                    dcc.Tab(id='Specs-tab',label='Tab Two', value='tab-2-example',className="custom-tab",
                            selected_className="custom-tab--selected",),        
                ],
            )
        ],
...
def build_top_panel(contri_data,contri_cols):
    ...
    html.Div(id='metric-rows',className='row',children=[dcc.Graph(id='decomp_chart'),]),
    ...
    if tab_switch == "tab2":
        ...
        html.Div(
            ...
            children=[
                ...
                html.Div(
                    id="graphs-container",
                    children=[build_top_panel(contri_data,contri_cols)],
                ),
            ],
        ),
...
def upload_file_and_create_contri_data_table(uploaded_filenames,uploaded_file_contents):
    ...
    if uploaded_filenames is None:
        ...
        if len(files) == 0:
            ...
            print("line 614")
            summary_dict,summary_cols, dfl_dict,dfl_cols, dfa_dict,dfa_cols,params_col=read_file_to_update_hiden_divs(file_name)
            
            return [html.Li(file_name),summary_dict,summary_cols, dfl_dict,dfl_cols, dfa_dict,dfa_cols,params_col]
...
def read_file_to_update_hiden_divs(file_name):
    ...
    time_grain=pd.read_excel(os.path.join(APP_PATH, os.path.join("data", file_name)),sheet_name="KPI")
    time_grain=time_grain['Time_Grain'][0]
    ...
    return summary_dict,summary_cols, dfl_dict,dfl_cols, dfa_dict,dfa_cols,params_col
...
def prepare_and_compute_decomp_data(file_name,contri_table_data):
    ...
    temp_df['percentage_normalized']=contri_table_data['percentage_normalized']
...
@app.callback(
    ...
    [Input('tv-column', 'value'),Input('dependent-column', 'value'),Input('var1', 'value'),Input('var2', 'value'),
     ...
     [State('summary_table','data'),State('dfl_table','data'),])
def regression(tv_var, depen, var1, var2, var3, var4, l1, l2, l3, l4, l5, base, var5, var6, var7, var8, var9, var10, l6, l7, l8, l9, l10, l11, summary_dict, dfl_dict):
         ...
         return columns, data ,data_to_temp_contri_table,columns_to_temp_contri_table
...
@app.callback([Output('contri_table','data'),Output('contri_table','columns')],[Input('freeze_contribution','n_clicks')],
               [State('temp_contri_table','data')])
def update_contri_table_from_tab1(n_clicks,contribution_dict):
...
@app.callback(
    ...
    [Input('plot_independent', 'value')],[State('dfa_table','data')]
    )

def plot2(indepen,dfa_dict):
    # Create figure with secondary y-axis
    dfa=pd.DataFrame(dfa_dict)
